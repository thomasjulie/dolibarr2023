...
/**
 * \file    customizeprospectcustomerfield/class/actions_customizeprospectcustomerfield.class.php
...
class ActionsCustomizeProspectCustomerField
{
	...
	public function addMoreMassActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		/* print_r($parameters); print_r($object); echo "action: " . $action; */
		if (in_array($parameters['currentcontext'], array('somecontext1', 'somecontext2'))) {		// do something only for the context 'somecontext1' or 'somecontext2'
			$this->resprints = '<option value="0"'.($disabled ? ' disabled="disabled"' : '').'>'.$langs->trans("CustomizeProspectCustomerFieldMassAction").'</option>';
		}
	...
	public function loadDataForCustomReports($parameters, &$action, $hookmanager)
	{
		...
		if ($parameters['tabfamily'] == 'customizeprospectcustomerfield') {
			$head[$h][0] = dol_buildpath('/module/index.php', 1);
			$head[$h][1] = $langs->trans("Home");
			$head[$h][2] = 'home';
			$h++;

			$this->results['title'] = $langs->trans("CustomizeProspectCustomerField");
			$this->results['picto'] = 'customizeprospectcustomerfield@customizeprospectcustomerfield';
		}
	...
	public function restrictedArea($parameters, &$action, $hookmanager)
	{
		...
		if ($parameters['features'] == 'myobject') {
			if ($user->rights->customizeprospectcustomerfield->myobject->read) {
	...
	public function completeTabsHead(&$parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($parameters['mode'] == 'remove') {
			...
			if (in_array($element, ['context1', 'context2'])) {
				...
				$parameters['head'][$counter][0] = dol_buildpath('/customizeprospectcustomerfield/customizeprospectcustomerfield_tab.php', 1) . '?id=' . $id . '&amp;module='.$element;
				$parameters['head'][$counter][1] = $langs->trans('CustomizeProspectCustomerFieldTab');
				if ($datacount > 0) {
					$parameters['head'][$counter][1] .= '<span class="badge marginleftonlyshort">' . $datacount . '</span>';
				}
				$parameters['head'][$counter][2] = 'customizeprospectcustomerfieldemails';
				$counter++;
			}
