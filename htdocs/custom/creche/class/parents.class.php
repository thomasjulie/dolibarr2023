...
class Parents extends CommonObject
{
	 ...
	 */
	public $table_element = 'creche_parents';
	...
	/**
	 * @var string String with name of icon for parents. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'parents@creche' if picto is file 'img/object_parents.png'.
	 */
	...
	public $fields=array(
		...
		'tel_portable' => array('type'=>'varchar(20)', 'label'=>'Telportable', 'enabled'=>'1', 'position'=>50, 'notnull'=>1, 'visible'=>-1, 'searchall'=>1,),
		'mail' => array('type'=>'varchar(150)', 'label'=>'Mail', 'enabled'=>'1', 'position'=>55, 'notnull'=>1, 'visible'=>-1, 'searchall'=>1,),
		'login' => array('type'=>'varchar(100)', 'label'=>'Login', 'enabled'=>'1', 'position'=>60, 'notnull'=>1, 'visible'=>-1, 'searchall'=>1,),
	...
	//  */
	// public $table_element_line = 'creche_parentsline';
	...
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Parents', 'fk_element'=>'fk_parents'));
	...
	//  */
	// protected $childtablesoncascade = array('creche_parentsdet');
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('creche', 'parents', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && !empty($user->rights->creche->parents->write))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && !empty($user->rights->creche->parents->parents_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->creche->dir_output.'/parents/'.$oldref;
				$dirdest = $conf->creche->dir_output.'/parents/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->creche->dir_output.'/parents/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowParents")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Parents").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/creche/parents_card.php', 1).'?id='.$this->id;
	...
	public function getNextNumRef()
	{
		...
		if (empty(getDolGlobalString('CRECHE_MYOBJECT_ADDON'))) {
			$conf->global->CRECHE_MYOBJECT_ADDON = 'mod_parents_standard';
		}
