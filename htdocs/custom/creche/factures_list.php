...
/**
 *   	\file       factures_list.php
 *		\ingroup    creche
 *		\brief      List page for factures
 */
...
require_once __DIR__.'/class/factures.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('creche', 'factures', 'write');
	$permissiontodelete = $user->hasRight('creche', 'factures', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Factures';
	$objectlabel = 'Factures';
	$uploaddir = $conf->creche->dir_output;
...
$title = $langs->trans("Factures");
//$help_url = "EN:Module_Factures|FR:Module_Factures_FR|ES:Módulo_Factures";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/creche/factures_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$topicmail = "SendFacturesRef";
$modelmail = "factures";
$objecttmp = new Factures($db);
