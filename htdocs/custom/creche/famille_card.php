...
$object->fields['date_create']['notnull'] = 0;
$object->fields['date_update']['notnull'] = 0;
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('creche', 'famille', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('creche', 'famille', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('creche', 'famille', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->creche->multidir_output[isset($object->entity) ? $object->entity : 1].'/famille';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/creche/famille_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/creche/famille_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
...
$title = $langs->trans("Famille")." - ".$langs->trans('Card');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	// Affichage des onglets en haut de la card
	print dol_get_fiche_head($head, 'card', $langs->trans("Famille"), -1, $object->picto, 0, '', '', 0, '', 1);
	...
	if ($action == 'xxx') {
		$text = $langs->trans('ConfirmActionFamille', $object->ref);
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/creche/famille_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	*/
	...
	dol_banner_tab($object, 'ref', $linkback, 1, 'ref', 'libelle', $morehtmlref);
	...
	if ($action == 'presend') {
		...
		$formmail->fromtype = (GETPOST('fromtype') ? GETPOST('fromtype') : 
			(!empty($conf->global->MAIN_MAIL_DEFAULT_FROMTYPE) ? $conf->global->MAIN_MAIL_DEFAULT_FROMTYPE : 'user'));
		...
		$formmail->fromname = $user->lastname;
		...
		$substitutionarray = array(
			...
			'__USER_FULLNAME__' => $user->lastname,
			'__USER_ID__' => $user->id
		);
		complete_substitutions_array($substitutionarray, $langs);

		$liste['famille'] = $object->libelle . ' <' . $object->mail . '>';
		...
		$formmail->withtopic = 'Information';
		$formmail->withfile = 2;
		$formmail->withbody = 1;
	...
	if ($action == 'send') {
		...
		$sendto = (GETPOST('sendto') != '') ? $object->mail . ',' . GETPOST('sendto') : $object->mail;
