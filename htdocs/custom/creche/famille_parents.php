...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('creche', 'parents', 'write');
	$permissiontodelete = $user->hasRight('creche', 'parents', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
$title = $langs->trans("Famille")." - ".$langs->trans('Parents');
llxHeader('', $title);

$head = famillePrepareHead($fam);
print dol_get_fiche_head($head, 'parents', $langs->trans("Famille"), -1, $fam->picto, 0, '', '', 0, '', 1);
		...
		<table class="tagtable liste">
			<thead>
				<tr class="liste_titre">
					<th class="wrapcolumntitle liste_titre"><?= $langs->trans("Civilité") ?></th>
					<th class="wrapcolumntitle liste_titre"><?= $langs->trans("Nom") ?></th>
					<th class="wrapcolumntitle liste_titre"><?= $langs->trans("Adresse") ?></th>
					<th class="wrapcolumntitle liste_titre"><?= $langs->trans("Code Postal") ?></th>
					<th class="wrapcolumntitle liste_titre"><?= $langs->trans("Ville") ?></th>
				</tr>
			</thead>
			<tbody>
				<?php while ($res = $db->fetch_object($req)): ?>
					<tr>
						<td><?= ucfirst($res->civilite) ?></td>
						<td>
							<a href="/custom/creche/parents_card.php?id=<?= $res->rowid ?>">
								<?= $res->prenom . ' ' . $res->nom ?>
							</a>
						</td>
						<td><?= $res->adresse ?></td>
