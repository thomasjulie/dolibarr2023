...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->rights->creche->parents->write;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
}
...
if ($object->id > 0) {
	$title = $langs->trans("Parents")." - ".$langs->trans('Agenda');
	...
	if (isModEnabled('agenda') && ($user->hasRight('agenda', 'myactions', 'read') || $user->hasRight('agenda', 'allactions', 'read'))) {
		...
		$cachekey = 'count_events_parents_'.$object->id;
		$nbEvent = dol_getcache($cachekey);

		print_barre_liste($langs->trans("ActionsOnParents").(is_numeric($nbEvent) ? '<span class="opacitymedium colorblack paddingleft">('.$nbEvent.')</span>': ''), 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlright, '', 0, 1, 1);
		*/
		print_barre_liste($langs->trans("ActionsOnParents"), 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlright, '', 0, 1, 0);
